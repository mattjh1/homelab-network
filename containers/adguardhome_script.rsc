...
:local cDefaultStorageMount "/usb1-part1"
:local cRootDir ($cDefaultStorageMount . "/agh")
:local cTmpDir ($cDefaultStorageMount . "/tmp")
:local cMountListConf "agh_conf"
:local cMountListWork "agh_work"
:local cMountSrc ($cDefaultStorageMount . "/conf/agh")
:local cMountWorkSrc ($cDefaultStorageMount . "/conf/agh/work")
:local cMountDst "/opt/adguardhome/conf"
:local cMountWorkDst "/opt/adguardhome/work"
...
:local mountExists [:len [/container mounts find list=$cMountListConf]]
:if ($mountExists = 0) do={
    /container mounts add list=$cMountListConf src=$cMountSrc dst=$cMountDst
    :put ("      |-- [OK] Created mount '" . $cMountListConf . "'")
} else={
    :put ("      |-- [OK] Mount '" . $cMountListConf . "' exists")
}

:local workMountExists [:len [/container mounts find list=$cMountListWork]]
:if ($workMountExists = 0) do={
    /container mounts add list=$cMountListWork src=$cMountWorkSrc dst=$cMountWorkDst
    :put ("      |-- [OK] Created mount '" . $cMountListWork . "'")
} else={
    :put ("      |-- [OK] Mount '" . $cMountListWork . "' exists")
}
...
:if ($containerExists = true) do={
    ...
    /container repull [find where name=$cName]
    ...
    /container add remote-image=$cImage name=$cName \
        interface=$cInterface logging=yes mountlists="$cMountListConf,$cMountListWork" start-on-boot=yes \
        root-dir=$cRootDir workdir=$cMountWorkDst \
